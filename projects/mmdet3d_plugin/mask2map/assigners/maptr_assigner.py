...
class MapTRAssigner(BaseAssigner):
    ...
    def assign(self, bbox_pred, cls_pred, pts_pred, gt_bboxes, gt_labels, gt_pts, gt_bboxes_ignore=None, eps=1e-7):
        ...
        normalized_gt_pts = normalize_2d_pts(gt_pts, self.pc_range) if not self.z_cfg["gt_z_flag"] else normalize_3d_pts(gt_pts, self.pc_range)
    ...
    def assign_dist(self, bbox_pred, cls_pred, pts_pred, gt_bboxes, gt_labels, gt_pts, gt_pts_mask, gt_bboxes_ignore=None, eps=1e-7):
        ...
        # num_q, num_pts, 2 <-> num_gt, num_pts, 2
        pts_cost_ordered = self.pts_cost(pts_pred_interpolated, normalized_gt_pts)
        pts_cost_ordered = pts_cost_ordered * 5.0 / pts_cost_ordered.max()
        gt_pts_mask_sum = (((1 - gt_pts_mask[..., 0].float()).sum(-1) == 60).float()) * 10000000
        pts_cost_ordered = pts_cost_ordered.view(num_bboxes, num_gts, num_orders)
        pts_cost_ordered = pts_cost_ordered + gt_pts_mask_sum.unsqueeze(0)
        pts_cost, order_index = torch.min(pts_cost_ordered, 2)
