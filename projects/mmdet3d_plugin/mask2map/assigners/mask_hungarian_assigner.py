...
class MaskHungarianAssigner(BaseAssigner):
    ...
    def assign(self, cls_pred, mask_pred, gt_labels, gt_mask, gt_bboxes_ignore=None, eps=1e-7):
...
@BBOX_ASSIGNERS.register_module()
class MaskHungarianAssignerShift(BaseAssigner):
    """Computes one-to-one matching between predictions and ground truth for
    mask.
    ...
    def assign(self, cls_pred, mask_pred, gt_labels, gt_mask, gt_bboxes_ignore=None, eps=1e-7):
        ...
        if self.mask_cost.weight != 0:
            ...
            # gt_mask shape = [num_gt, h, w]
            # mask_cost shape = [num_query, num_gt]
            mask_cost = self.mask_cost(mask_pred, gt_mask > 0)
        else:
            mask_cost = 0

        if self.dice_cost.weight != 0:
            dice_cost = self.dice_cost(mask_pred, gt_mask > 0)
        else:
            dice_cost = 0
        padding_idx = gt_mask.flatten(1).sum(1) == 0
        mask_ordered = mask_cost + dice_cost
        mask_ordered[:, padding_idx] = 10000
        mask_ordered = mask_ordered.unflatten(1, [num_query, -1])
        mask_ordered_cost, order_index = mask_ordered.min(2)
        cost = cls_cost + mask_ordered_cost
        ...
        assigned_gt_inds[matched_row_inds] = matched_col_inds + 1
        assigned_labels[matched_row_inds] = gt_labels[matched_col_inds]
        return AssignResult(num_gt, assigned_gt_inds, None, labels=assigned_labels), order_index
