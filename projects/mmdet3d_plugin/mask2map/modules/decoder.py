        ...
        **kwargs,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                ...
                if output.shape[1] < 500:
                    bs, num_query, num_pts, code_size = reference_points.shape
                    tmp = tmp.view(bs, num_query, num_pts, code_size)
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        if attn_masks is None:
            ...
            assert len(attn_masks) == self.num_attn, (
                f"The length of " f"attn_masks {len(attn_masks)} must be equal " f"to the number of attention in " f"operation_order {self.num_attn}"
            )
...
@TRANSFORMER_LAYER.register_module()
class DecoupledDetrTransformerDecoderLayer_CP(BaseTransformerLayer):
    """Implements decoder layer in DETR transformer.
    Args:
        attn_cfgs (list[`mmcv.ConfigDict`] | list[dict] | dict )):
            Configs for self_attention or cross_attention, the order
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DecoupledDetrTransformerDecoderLayer_CP, self).__init__(
            attn_cfgs=attn_cfgs,
            feedforward_channels=feedforward_channels,
            ffn_dropout=ffn_dropout,
            operation_order=operation_order,
    ...
    def forward(
        ...
        **kwargs,
        ...
        cls_branches = kwargs["cls_branches"]
        spatial_shapes = kwargs["spatial_shapes"]
        level_start_index = kwargs["level_start_index"]
        mlvl_feats = kwargs["mlvl_feats"]
        feat_flatten = kwargs["feat_flatten"]
        feat_spatial_shapes = kwargs["feat_spatial_shapes"]
        feat_level_start_index = kwargs["feat_level_start_index"]
