...
class BaseTransform(BaseModule):
    ...
    @force_fp32()
    def get_geometry(self, fH, fW, lidar2img, img_metas):
...
class BaseTransformV2(BaseModule):
    def __init__(
        ...
        self.dbound = dbound
        self.sid = sid
        self.frustum = self.create_frustum(dbound, input_size, feat_down_sample)
        self.C = out_channels
    ...
    def create_frustum(self, depth_cfg, input_size, downsample):
        ...
        if self.sid:
            ...
            d_sid = torch.exp(torch.log(depth_cfg_t[0]) + d_sid / (self.D - 1) * torch.log((depth_cfg_t[1] - 1) / depth_cfg_t[0]))
    ...
    def get_lidar_coor(self, fH, fW, rots, trans, intrins, post_rots, post_trans, lidar2ego_rots, lidar2ego_trans, img_metas):
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        if ranks_feat is None:
            ...
            dummy = torch.zeros(size=[feat.shape[0], feat.shape[2], int(self.grid_size[2]), int(self.grid_size[0]), int(self.grid_size[1])]).to(feat)
    ...
    def forward(self, images, img_metas):
        ...
        coor = self.get_geometry_v1(fH, fW, rots, trans, intrins, post_rots, post_trans, lidar2ego_rots, lidar2ego_trans, img_metas)
        mlp_input = self.get_mlp_input(camera2ego, camera_intrinsics, post_rots, post_trans)
    ...
    def __init__(
        ...
        if use_dcn:
            depth_conv_list.append(
                build_conv_layer(
                    cfg=dict(type="DCN", in_channels=mid_channels, out_channels=mid_channels, kernel_size=3, padding=1, groups=4, im2col_step=128)
                )
            )
...
class BEVFormerEncoderDepth(BEVFormerEncoder):
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        gt_depths = gt_depths.view(B * N, H // self.feat_down_sample, self.feat_down_sample, W // self.feat_down_sample, self.feat_down_sample, 1)
...
class LSSTransform(BaseTransform):
    ...
    def forward(self, images, img_metas):
        ...
        x = self.downsample(x)
        ret_dict = dict(bev=x, depth=depth)
        return ret_dict
...
class LSSTransformV2(BaseTransformV2):
    def __init__(
        ...
        self.loss_depth_weight = loss_depth_weight
        self.grid_config = grid_config
        self.depth_net = DepthNet(self.in_channels, self.in_channels, self.C, self.D, **depthnet_cfg)
        if downsample > 1:
            assert downsample == 2, downsample
            self.downsample = nn.Sequential(
                nn.Conv2d(out_channels, out_channels, 3, padding=1, bias=False),
...
class SimpleBEVEncoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.inc = nn.Conv2d(in_channels, in_channels, 3, 1, 1)
        self.down1 = Down(in_channels, in_channels)
        self.down2 = Down(in_channels, in_channels)
        self.down3 = Down(in_channels, out_channels)
