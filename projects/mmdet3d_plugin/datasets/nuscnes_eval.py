...
def class_tp_curve(
    md_list: DetectionMetricDataList, metrics: DetectionMetrics, detection_name: str, min_recall: float, dist_th_tp: float, savepath: str = None, ax: Axis = None
) -> None:
    """
    ...
    # Prepare axis.
    if ax is None:
        ax = setup_axis(title=PRETTY_DETECTION_NAMES[detection_name], xlabel="Recall", ylabel="Error", xlim=1, min_recall=min_recall)
...
def exist_corners_in_image_but_not_all(box, intrinsic: np.ndarray, imsize: Tuple[int, int], vis_level: int = BoxVisibility.ANY) -> bool:
    """
...
def filter_eval_boxes_by_id(nusc: NuScenes, eval_boxes: EvalBoxes, id=None, verbose: bool = False) -> EvalBoxes:
    """
...
def filter_eval_boxes_by_visibility(ori_eval_boxes: EvalBoxes, visibility=None, verbose: bool = False) -> EvalBoxes:
    """
    Applies filtering to boxes. Distance, bike-racks and points per box.
    :param nusc: An instance of the NuScenes class.
    :param eval_boxes: An instance of the EvalBoxes class.
...
def filter_eval_boxes_by_overlap(nusc: NuScenes, eval_boxes: EvalBoxes, verbose: bool = False) -> EvalBoxes:
    """
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        for box in eval_boxes[sample_token]:
            ...
            for cam in cams:
                ...
                new_box = Box(box.translation, box.size, Quaternion(box.rotation), name=box.detection_name, token="")
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def render(self, metrics: DetectionMetrics, md_list: DetectionMetricDataList) -> None:
        ...
        summary_plot(
            md_list, metrics, min_precision=self.cfg.min_precision, min_recall=self.cfg.min_recall, dist_th_tp=self.cfg.dist_th_tp, savepath=savepath("summary")
        )
...
if __name__ == "__main__":
    ...
    # Settings.
    parser = argparse.ArgumentParser(description="Evaluate nuScenes detection results.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("result_path", type=str, help="The submission as a JSON file.")
    parser.add_argument("--output_dir", type=str, default="~/nuscenes-metrics", help="Folder to store result metrics, graphs and example visualizations.")
    parser.add_argument("--eval_set", type=str, default="val", help="Which dataset split to evaluate on, train, val or test.")
    parser.add_argument("--dataroot", type=str, default="data/nuscenes", help="Default nuScenes data directory.")
    ...
    parser.add_argument("--config_path", type=str, default="", help="Path to the configuration file." "If no path given, the CVPR 2019 configuration will be used.")
    parser.add_argument("--plot_examples", type=int, default=0, help="How many example visualizations to write to disk.")
    parser.add_argument("--render_curves", type=int, default=1, help="Whether to render PR and TP curves to disk.")
    parser.add_argument("--verbose", type=int, default=1, help="Whether to print to stdout.")
    ...
    nusc_eval = NuScenesEval_custom(nusc_, config=cfg_, result_path=result_path_, eval_set=eval_set_, output_dir=output_dir_, verbose=verbose_)
    for vis in ["1", "2", "3", "4"]:
