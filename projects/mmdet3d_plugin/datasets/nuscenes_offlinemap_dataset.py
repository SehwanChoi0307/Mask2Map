...
class LiDARInstanceLines(object):
    ...
    def start_end_points(self):
        ...
        instance_se_points_tensor = instance_se_points_tensor.to(dtype=torch.float32)
    ...
    def line_ego_to_mask(self, line_ego, mask, color=1, thickness=3):
                ...
                if instance_label == 3:
                    ...
                    if is_poly:
                        ...
                        for shift_right_i in range(shift_num):
                            ...
                            shift_pts = np.concatenate((shift_pts, pts_to_concat), axis=0)
                            shift_instance = LineString(shift_pts)
                            shift_sampled_points = np.array([list(shift_instance.interpolate(distance).coords) for distance in distances]).reshape(
                                -1, 2
                            )
                            shift_pts_list.append(shift_sampled_points)
                        # import pdb;pdb.set_trace()
...
class VectorizedLocalMap(object):
    CLASS2LABEL = {"divider": 0, "ped_crossing": 1, "boundary": 2, "centerline": 3, "others": -1}
    ...
    def line_ego_to_pvmask(self, line_ego, mask, lidar2feat, color=1, thickness=1, z=-1.6):
...
class CustomNuScenesOfflineLocalMapDataset(CustomNuScenesDataset):
            ...
            if self.sequences_split_num == "all":
                ...
                for curr_flag in range(len(bin_counts)):
                    curr_sequence_length = np.array(
                        list(range(0, bin_counts[curr_flag], math.ceil(bin_counts[curr_flag] / self.sequences_split_num))) + [bin_counts[curr_flag]]
                    )
    ...
    def prepare_train_data(self, index):
        ...
        if self.dn_enabled:
            example["img_metas"].data.update({"gt_bboxes_3d": example["gt_bboxes_3d"], "gt_labels_3d": example["gt_labels_3d"]})
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            try:
                ref_logits = det["ref_logits"].detach().cpu().numpy()
                pred_anno["ref_logits"] = ref_logits
            except:
                pass
            sample_token = self.data_infos[sample_id]["token"]
            ...
            for i, vec in enumerate(vecs):
                ...
                anno = dict(pts=vec["pts"], pts_num=len(vec["pts"]), cls_name=name, type=vec["label"], confidence_level=vec["score"])
    ...
    def to_gt_vectors(self, gt_dict):
        ...
        for gt_instance, gt_label in zip(gt_instances, gt_labels):
            ...
            gt_vectors.append({"pts": pts, "pts_num": pts_num, "type": int(gt_label)})
        vector_num_list = {}
    ...
    def _evaluate_single(self, result_path, logger=None, metric="chamfer", result_name="pts_bbox"):
        ...
        for metric in metrics:
            ...
            if metric == "chamfer":
                ...
                thresholds = np.linspace(0.5, 0.95, int(np.round((0.95 - 0.5) / 0.05)) + 1, endpoint=True)
    ...
    def Vector_evaluate(self, pred_vectors, scores, groundtruth, thresholds, metric):
    ...
    def interp_fixed_dist(self, vector, sample_dist):
    ...
    def _evaluate_single_dist(self, result_path, logger=None, metric="chamfer", result_name="pts_bbox"):
        ...
        # print results
        table = prettytable.PrettyTable(["category", "num_preds", "num_gts"] + [f"AP@{thr}" for thr in THRESHOLDS] + ["AP"])
        for label in id2cat.keys():
            table.add_row(
                [
                    id2cat[label],
    ...
    def evaluate_dist(
        self, results, metric="bbox", logger=None, jsonfile_prefix=None, result_names=["pts_bbox"], show=False, out_dir=None, pipeline=None
    ):
        result_files, tmp_dir = self.format_results(results, jsonfile_prefix)
