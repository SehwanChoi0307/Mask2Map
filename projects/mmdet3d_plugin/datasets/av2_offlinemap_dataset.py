...
class LiDARInstanceLines(object):
    ...
    def fixed_num_sampled_points(self):
        ...
        instance_points_tensor[:, :, 2] = torch.clamp(
            instance_points_tensor[:, :, 2], min=self.min_z, max=self.max_z
        )
    ...
    def line_ego_to_mask(self, line_ego, mask, color=1, thickness=3):
        ...
        """
        trans_x = self.canvas_size[1] / 2  # 100
        trans_y = self.canvas_size[0] / 2  # 50
    ...
    def fixed_num_sampled_points_condi(self):
        ...
        return instance_points_tensor[..., : self.code_size]
    ...
    def fixed_num_sampled_points_ambiguity(self):
        ...
        instance_points_tensor = (
            instance_points_tensor if is_3d else instance_points_tensor[:, :, :2]
        )
    ...
    def shift_fixed_num_sampled_points(self):
        ...
        for fixed_num_pts in fixed_num_sampled_points:
            ...
            if not is_poly:
                padding = torch.full(
                    [fixed_num - shift_pts.shape[0], fixed_num, shift_pts.shape[-1]],
                    self.padding_value,
                )
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            ...
            multi_shifts_pts_tensor[:, :, 2] = torch.clamp(
                multi_shifts_pts_tensor[:, :, 2], min=self.min_z, max=self.max_z
            )
            ...
            if multi_shifts_pts_tensor.shape[0] < final_shift_num:
                padding = torch.full(
                    [
                        ...
                        self.fixed_num,
                        multi_shifts_pts_tensor.shape[-1],
                    ],
                    self.padding_value,
                )
    ...
    def instance_segments_condi(self):
        ...
        for instance in self.instance_list:
            ...
            if instance.has_z:
                ...
                sampled_points = sampled_points.reshape(-1, 2)
            instance_points_list.append(sampled_points[..., : self.code_size])
...
class VectorizedAV2LocalMap(object):
    ...
    def __init__(
        ...
        thickness=3,
        aux_seg=dict(
            use_aux_seg=False, bev_seg=False, pv_seg=False, seg_classes=1, feat_down_sample=32
        ),
    ):
        """
        Args:
        ...
        for vec_class in self.vec_classes:
            ...
            for instance in instance_list:
                if instance.shape[0] < 2:
                    ...
                    continue
                vectors.append(
                    (LineString(np.array(instance)), self.CLASS2LABEL.get(vec_class, -1))
                )
        filtered_vectors = []
        gt_pts_loc_3d = []
        gt_pts_num_3d = []
            ...
            if self.aux_seg["seg_classes"] == 1:
                if self.aux_seg["bev_seg"]:
                    gt_semantic_mask = np.zeros(
                        (1, self.canvas_size[0], self.canvas_size[1]), dtype=np.uint8
                    )
                else:
                        ...
                        if instance.geom_type == "LineString":
                            if self.aux_seg["bev_seg"]:
                                self.line_ego_to_mask(
                                    instance, gt_semantic_mask[0], color=1, thickness=self.thickness
                                )
                            if self.aux_seg["pv_seg"]:
                ...
                if self.aux_seg["bev_seg"]:
                    gt_semantic_mask = np.zeros(
                        (len(self.vec_classes), self.canvas_size[0], self.canvas_size[1]),
                        dtype=np.uint8,
                    )
                else:
                    gt_semantic_mask = None
    ...
    def line_ego_to_pvmask(self, line_ego, mask, lidar2feat, color=1, thickness=1):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        anns_results = self.vector_map.gen_vectorized_samples(
            (
                ...
                if "annotation" in input_dict.keys()
    ...
    def prepare_train_data(self, index):
        ...
        if self.dn_enabled:
            example["img_metas"].data.update(
                {"gt_bboxes_3d": example["gt_bboxes_3d"], "gt_labels_3d": example["gt_labels_3d"]}
            )
                ...
                for j, thr in enumerate(thresholds):
                    if metric == "chamfer":
                        ...
                        if thr == 0.5 or thr == 0.75:
                            detail["AV2Map_{}/{}_AP_thr_{}".format(metric, name, thr)] = cls_aps[j][
                                i
                            ]
