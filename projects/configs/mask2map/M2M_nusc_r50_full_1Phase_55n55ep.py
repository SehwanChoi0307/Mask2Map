...
model = dict(
    ...
    pts_bbox_head=dict(
        type='MapTRv2Head_mask_F1_only_mask_DN',
        bev_h=bev_h_,
        bev_w=bev_w_,
        num_query=900,
        num_vec_one2one=50,
        ...
        # z_cfg=z_cfg,
        transformer=dict(
            type='MapTRPerceptionTransformer_mask_tempF_DN_CP_ptsdfor_rv',
            ...
            dn_group_num=5,
            dn_instance_noise="shift_scale",
            dn_noise_scale=0.01,
            dn_pts_noise="pred_sampling",
            use_instance_query_embed=False,
            query_embed_pos_type="mask_weight_avg",
            thresh_of_mask_for_pos=0.3,
            MaskPosNone=True,
            query_cls_embed=True,
            query_cls_embed_with_background=False,
            gt_mask_for_pos_embed=True,
            ...
            bev_neck=dict(
                ...
                encoder=dict(  # DeformableDetrTransformerEncoder
                    ...
                    layer_cfg=dict(  # DeformableDetrTransformerEncoderLayer
                        self_attn_cfg=dict(embed_dims=256, num_heads=8, num_levels=3, num_points=4, dropout=0.0,
                                           batch_first=True),  # MultiScaleDeformableAttention
                        ffn_cfg=dict(embed_dims=256, feedforward_channels=1024, num_fcs=2, ffn_drop=0.0,
            ...
            segm_decoder=dict(  # Mask2FormerTransformerDecoder
...
lr_config = dict(
    ...
    min_lr_ratio=1e-3)
total_epochs = 55
evaluation = dict(interval=255, pipeline=test_pipeline, metric='chamfer',
